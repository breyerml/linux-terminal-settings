...
function _warp_add() {
    ...
    if [ -z $warp_point_exists ]; then
        sed -i "$ a $1=$PWD" $warp_file_dir                         ## append new warp point after the last line
        echo "warp point added: $1 → $PWD"
    else 
        echoerr "warp: $1 already added (use --warp-add! to override)"
        return 17
    fi
}
...
function _warp_force_add() {
    ...
    if [ -z $warp_point_exists ]; then
        ...
        echo "replaced warp point: $2: ${old_warp_dir#*=} ⇒ $PWD"
    fi
}
...
function _warp_remove() {
    ...
    if [ -n $warp_point_exists ]; then 
       ...
       echoerr "warp: $2 doesn't exist (use --warp-list to list all available warp points)"
       return 2
    fi
}
